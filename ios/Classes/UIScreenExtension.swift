...
private func computeIfSome<T: Any, S: Any>(optional: T?, computation: ((T) -> S)) -> S? {
  if let some = optional { return computation(some) } else { return .none }
}
...
extension UIScreen {
  ...
  public static let pixelsPerInch: CGFloat? = computeIfSome(
    optional: dimensionInInches, computation: { main.nativeBounds.height / $0.height })

  /// The number of pixels per centimeter for this device
  public static let pixelsPerCentimeter: CGFloat? = computeIfSome(
    optional: pixelsPerInch, computation: { $0 / 2.54 })

  /// The number of points per inch for this device
  public static let pointsPerInch: CGFloat? = computeIfSome(
    optional: pixelsPerInch, computation: { $0 / main.nativeScale })

  /// The number of points per centimeter for this device
  public static let pointsPerCentimeter: CGFloat? = computeIfSome(
    optional: pixelsPerCentimeter, computation: { $0 / main.nativeScale })

  /// The screen dimension in inches
  public static let dimensionInInches: CGSize? = computeIfSome(
    optional: diagonalInInches,
    computation: {
  ...
  public static let dimensionInCentimeters: CGSize? = computeIfSome(
    optional: pixelsPerCentimeter,
    computation: {
      CGSize(width: main.nativeBounds.width / $0, height: main.nativeBounds.height / $0)
    })
